...
def run_dotnet_build(
    ...
    """Runs `dotnet publish` and returns the expected output directory"""
    # Путь теперь ведет в bin/Config/Framework/publish
    output_dir = project_path.parent / "bin" / config / framework / "publish"
    ...
    if result.returncode != 0:
        err(f"Build output:\n{result.stdout}")
        err(f"Build errors:\n{result.stderr}")
        raise RuntimeError(f"dotnet publish failed (exit code {result.returncode})")

    if not output_dir.exists():
        # Если папка publish не создалась (такое бывает в старых SDK), 
        # откатываемся на уровень выше, но обычно в net10.0 она будет.
        fallback_dir = output_dir.parent
...
def package_module(
    ...
    try:
        ...
        with zipfile.ZipFile(tmp_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6) as zf:
            ...
            zf.writestr("bbmanifest.json", manifest_json)
...
def build_and_package_module(
    ...
    info(f"Manifest  : bbmanifest.json")

    # 1. Build
    step("1/4  dotnet publish")
...
def main() -> int:
    ...
    log(f"  Manifest     : bbmanifest.json")
